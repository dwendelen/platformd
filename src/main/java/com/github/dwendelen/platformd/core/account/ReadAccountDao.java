...
public class ReadAccountDao {
    ...
    public ReadAccount getAccount(UUID uuid) {
        ResultSet results = session.execute(QueryBuilder.select("account_uuid", "account_name").distinct().from("account")
                .where(QueryBuilder.eq("account_uuid", uuid)));
        return StreamSupport.stream(results.spliterator(), false)
                .map(row -> {
                    ReadAccount readAccount = new ReadAccount();
                    readAccount.name = row.getString("account_name");
    ...
    public void on(NormalAccountCreated normalAccountCreated) {
        ...
        if (!BigDecimal.ZERO.equals(normalAccountCreated.initialBalance)) {
            session.execute(QueryBuilder.insertInto("account")
                    .value("account_uuid", normalAccountCreated.uuid)
                    .value("transaction_uuid", UUIDs.timeBased())
                    .value("amount", normalAccountCreated.initialBalance)
                    .value("comment", "Initial balance"));
        }
    }
